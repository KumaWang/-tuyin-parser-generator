...
{
    internal class MetadataProduction<TAction> : ProductionBase<TAction>
    {
        public override ProductionType ProductionType => ProductionType.Metadata;

        private ProductionBase<TAction> production;
        private TAction action;

        public MetadataProduction(ProductionBase<TAction> production, TAction action)
        {
        ...
        internal override IGraphEdgeStep<TMetadata> InternalCreate<TMetadata>(GraphFigure<TMetadata, TAction> figure, IGraphEdgeStep<TMetadata> last, IGraphEdgeStep<TMetadata> entry)
        {
            ...
            {
                var meta = figure.GraphBox.CreateMetadata(right, action, true);
