...
{
    interface IGraphEdgeStep<TMetadata>
    {
    ...
    abstract class BaseStep<TMetadata> : IGraphEdgeStep<TMetadata>
    {
    ...
    class Entry<TMetadata> : BaseStep<TMetadata>
    {
        ...
        internal Entry(GraphState<TMetadata>[] points) 
        {
            Start = points;
        }
    }
    ...
    class NextStep<TMetadata> : BaseStep<TMetadata>
    {
        ...
        internal NextStep(IGraphEdgeStep<TMetadata> left, IGraphEdgeStep<TMetadata> right)
        {
            Start = left.Start;
            End = right.End;
        }
    ...
    class ConnectStep<TMetadata> : BaseStep<TMetadata>
    {
        ...
        internal ConnectStep(IEnumerable<IGraphEdgeStep<TMetadata>> results)
        {
            Start = results.SelectMany(x => x.Start).Distinct().ToArray();
            End = results.SelectMany(x => x.End).Distinct().ToArray();
        }

        internal ConnectStep(IEnumerable<IGraphEdgeStep<TMetadata>> results, IEnumerable<GraphState<TMetadata>> otherEnds)
        {
            Start = results.SelectMany(x => x.Start).Distinct().ToArray();
