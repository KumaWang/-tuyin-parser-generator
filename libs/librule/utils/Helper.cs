...
namespace librule.utils
{
    ...
    {
        public static string GetUniqueName(this IEnumerable<Token> tokens)
        {
            // Todo:0 and 2 return same value bug

            var hashCode = 0;
            foreach (var token in tokens.SelectMany(x => x.Expands()).Distinct().Where(x => x.Expression.GetMaxLength() != 0).OrderBy(x => x.Index))
                hashCode = HashCode.Combine(hashCode, token.Index);
            ...
            {
                int index = hashCode & 0x3D;
                result[i] = chars[index >= 0 ? index : ~index + 1 & 0x3D];
                hashCode >>= 6;
            }
    ...
    static class Helper
    {
        ...
        public static IEnumerable<ProductionBase<TAction>> Scan<TAction>(this ProductionBase<TAction> production, Action<ProductionBase<TAction>> action = null, Func<ProductionBase<TAction>, bool> filter = null)
        {
            var visited = new HashSet<ProductionBase<TAction>>();
            var reutrns = new HashSet<ProductionBase<TAction>>();
            var stack = new Stack<ProductionBase<TAction>>();
