...
class Grammar
{
	...
	public Production this[string symbol]
	{
		get
		{
			...
			if (p != null) return p;
			p = new Production(this, symbol, "object");
			_production.Add(p);
			return p;
		}
	}
	...
	public void GenerateCode(List<U.Tuple<RegRoot, string>> lexerActions, ParserOptions po, U.CsStreamWriter tw)
	{
		...
		foreach (var p in _production)
		{
			if (p.FromLeftFactorize == 0)
				tw.WriteLine("{0} {1}(object {1}_i)", p.Type, p.Symbol);
			else
			{
				tw.Write("{0} {1}(", p.Type, p.Symbol);
				for (int kk = 0; kk < p.FromLeftFactorize; ++kk)
					tw.Write("object nt{0}_s, ", kk + 1);
				tw.WriteLine("object {0}_i)", p.Symbol, p.Type);
			}
			tw.WriteLine("{");
			...
			for (var a = 0; a < p.Alt.Count; ++a)
			{
				...
				if (true)
				{
					...
					{
						for (int i = 0; i < nnt; ++i)
							tw.WriteLine("object nt{0}_i = null;", i + 1 + p.FromLeftFactorize);
						tw.WriteLine();
